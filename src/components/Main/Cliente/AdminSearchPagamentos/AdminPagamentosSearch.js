...
const AdminPagamentosSearch = (props) => {
  ...
  const [versaointerface, setversaointerface] = useState(""); // Adicionado BELINI
  ...
  //  Para ajustar a visualização dos meses  -  BELINI
  const defaultPickerValue = [moment().subtract(1, 'month'), moment()];
  ...
  const getPreviousMonth = () => {  // Função para calcular o mês anterior
    const currentMonth = moment().month();
    const previousMonth = currentMonth === 0 ? 11 : currentMonth - 1; // Se for janeiro, mês anterior é dezembro
    return moment().month(previousMonth);
  };

  useEffect(() => {// Chama a função getPaymentsPeriod inicialmente com valores nulos
    getPaymentsPeriod(id, dataInicio, dataFim);
  }, []); // [] garante que useEffect seja executado apenas uma vez após a montagem inicial

  useEffect(() => {// useEffect para chamar getPaymentsPeriod sempre que dataInicio ou dataFim mudarem
  ...
  console.log("LOG 01:", `${process.env.REACT_APP_SERVIDOR}/Info-Administrador/${id}`);


  { /*  Envia requisição com ou sem datas de período defindas - BELINI */ }
  const getPaymentsPeriod = (id) => {
    setLoadingTable(true);

    const url = `${process.env.REACT_APP_SERVIDOR}/Info-Administrador/${id}`;
    ...
    console.log("URL completa  de consulta:", urlWithParams);
    ...
    axios
      ...
      .then((res) => {
        console.log('Response data da requisição:', res.data);
        ...
        setnumerodesaidas(res?.data?.numerodesaidas); // Pelúcia
        ...
        setversaointerface(res?.data?.versaointerface); // Adicionado BELINI
  ...
  const EditarCliente = () => {
    ...
    localStorage.setItem(`Numerostoryid`, store_id);  // grava em localStorage - Adicionado BELINI    
    localStorage.setItem(`Numeromaquina`, idmaquina);  // grava em localStorage - Adicionado BELINI    
    ...
    localStorage.setItem(`Interfaceversao`, versaointerface);  // grava em localStorage - Adicionado BELINI   
  ...
  const onRelatorioHandler = () => {
    if (!dataInicio && !dataFim) {
      ...
      navigate(`/cliente-maquinas/pagamentos/relatorio-adm/${id}`, {
        state: { maquinaInfos, clienteInfo, dataInicio, dataFim },
      });
    }
  };

  console.log("Rota Acessando info admin:", `/cliente-maquinas/pagamentos/relatorio-adm/${id}`);

  return (
    <div className="Admin_PagamentosSearch_container">
      {isLoading && <LoadingAction />}

      <div className="Admin_Container_atualizar">

        <div className="Admin_Dashboard_titulo">{`${nome} - ${descricao}`}</div>

        <Button className="Admin_Botao_atualizar" style={{ margin: "0 15px" }} onClick={handleRefresh}> {/* Atualiza a página do navegador, adicionado BELINI */}
          <FontAwesomeIcon
            icon={faArrowsRotate}
        ...
        <Button onClick={generatePdf} className="header_pdf">  {/* Gerar Pdf, adicionado BELINI */}
      ...
      <div className="Admin_PagamentosSearch_header">
        ...
        <Button
          className="header_editBtn"
          onClick={() => {
            navigate(`/cliente-maquinas/delete-all-pagamentos-adm/${id}`, {             
              state: location.state,
            });
          }}
        >
        ...
        <Button
          className="header_editBtn"
          onClick={() => {
            navigate(`/cliente-maquinas/credito-remoto-adm/${maquinaInfos.id}`, {
              state: location.state,
            });
          }}
        >
        ...
        <div className="PagamentosSearch_datePicker" ref={datePickerRef}>
      ...
      <div id="pdf-content"> {/* Salvar em pdf - BELINI   */}
        <div className="Admin_PagamentosSearch_body">
          ...
          <div className="Admin_PagamentosSearch_titleList_main"> {/* adicionado BELINI */}
            <div className="PagamentosSearch_pair"> {/* adicionado BELINI */}

              <div class="pair_admin">
                <div class="pair_text_admin">Entradas</div>
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(total)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Saidas</div>  {/* alterado BELINI*/}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(saidas)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Saldo</div> {/*. adicionado BELINI*/}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(saldo)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Cash</div> {/* alterado BELINI*/}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(cash)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">PIX</div> {/* adicionado BELINI */}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(pix)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Débito</div> {/* adicionado BELINI */}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(debito)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Crédito</div> {/* adicionado BELINI */}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(credito)}</div>
              </div>
           
              <div class="pair_admin">
                <div class="pair_text_admin">% Pix</div> {/* adicionado BELINI */}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(taxapix)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">% Débito</div> {/* adicionado BELINI */}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(taxadebito)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">% Crédito</div> {/* adicionado BELINI */}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(taxacredito)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Arrecadou Parcial</div> {/* adicionado BELINI */}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(arrecadouparcial)}</div>
              </div>

            </div>
          </div>

          <div className="Admin_PagamentosSearch_titleList_main2"> {/* adicionado BELINI */}
            <div className="PagamentosSearch_pair"> {/* adicionado BELINI */}

              <div class="pair_admin">
                <div class="pair_text_admin">Total Online</div> {/*. Adicionado BELINI*/}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(online)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Estornos</div>
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(estornos)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Estoque</div>
                <div className="pair_value_admin">{estoque ? Number(estoque) : 0}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Pulso Crédito</div>
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(valorDoPulso)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Pulso Jogada</div>
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(pulsotentativa)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Data Inicial</div>
                <div className="pair_value_admin"> {dataMaisAntiga} </div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Data Final</div>
                <div className="pair_value_admin"> {dataMaisRecente} </div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Nº de Dias</div>
                <div className="pair_value_admin"> {diasEntreDatas} </div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Livre</div>
                <div className="pair_value_admin"> 0 </div>
              </div>
          ...
          <div className="Admin_PagamentosSearch_titleList_main3"> 
            <div className="PagamentosSearch_pair"> {/* adicionado BELINI */}

              <div class="pair_admin">
                <div class="pair_text_admin">Saída Pelúcia</div>
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(saidapelucia)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Média Saídas</div> {/* adicionado BELINI */}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(mediasaidas)}</div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Limitador</div>
                <div className="pair_value_admin"> {limitetrava} </div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Nº de Jogadas</div>
                <div className="pair_value_admin">
                  {Intl.NumberFormat("pt-BR").format(Math.round(qtdetentativas))}
                </div>
              </div>

              <div class="pair_admin">
                <div class="pair_text_admin">Nº de Saídas</div>
                <div className="pair_value_admin">
              ...
              <div class="pair_admin">
                <div class="pair_text_admin">% de Saída</div>
                <div className="pair_value_admin">
              ...
              <div class="pair_admin">
                <div class="pair_text_admin">% de Lucro</div>
                <div className="pair_value_admin">
              ...
              <div class="pair_admin">
                <div class="pair_text_admin">Arrecadou Total</div> {/* adicionado BELINI */}
                <div className="pair_value_admin">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL", }).format(arrecadoutotal)}</div>
              </div>
          ...
          <div className="Admin_PagamentosSearch_tableContainer">
            <Table
              className="Admin_PagamentosSearch_table"
              columns={columns}
